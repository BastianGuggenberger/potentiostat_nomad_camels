...
stepnumber = 100
namespace["stepnumber"] = stepnumber
waitingtime = 0.01
namespace["waitingtime"] = waitingtime

Protocol_CV_Stat_variable_signal = variable_reading.Variable_Signal(name="Protocol_CV_Stat_variable_signal", variables_dict=namespace)
aliases = {}
...
def Protocol_CV_Stat_plan_inner(devs, stream_name="primary", runEngine=None):
    global cycles, last_V, max_V, min_V, start_V, stepnumber, waitingtime
    ...
    for CyclesLoop_Count, CyclesLoop_Value in enumerate(helper_functions.get_range(eva, "1", "cycles", "cycles", "nan", "nan", "start - stop", "linear", "True", "nan", False)):
        ...
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        for Voltage_Loop_Up_Count, Voltage_Loop_Up_Value in enumerate(helper_functions.get_range(eva, "min_V", "max_V", "stepnumber", "nan", "nan", "start - stop", "linear", "True", "nan", False)):
            ...
            helper_functions.update_protocol_counter(protocol_step_information)
            yield from bps.checkpoint()
            channels_Read_CV = [devs["Potentiostat"].U, devs["Potentiostat"].I_potstat, Protocol_CV_Stat_variable_signal]
        ...
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        for Voltage_Loop_Down_Count, Voltage_Loop_Down_Value in enumerate(helper_functions.get_range(eva, "max_V", "min_V", "stepnumber", "nan", "nan", "start - stop", "linear", "True", "nan", False)):
            ...
            helper_functions.update_protocol_counter(protocol_step_information)
            yield from bps.checkpoint()
            channels_Read_CV_1 = [devs["Potentiostat"].U, devs["Potentiostat"].I_potstat, Protocol_CV_Stat_variable_signal]
...
def Protocol_CV_Stat_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from Protocol_CV_Stat_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 3011
    md["user"] = {'name': 'default_user'}
    md["sample"] = {'name': 'default_sample'}
    md["session_name"] = session_name
    md["protocol_overview"] = """'CyclesLoop' for (start: 1, stop: cycles, points: cycles):\n\t'Voltage_Loop_Up' for (start: min_V, stop: max_V, points: stepnumber):\n\t\tSet Channels 'Set_U' - {'Potentiostat_U': 'Voltage_Loop_Up_Value'}\n\t\tWait 'Wait' - waitingtime s\n\t\tRead Channels 'Read_CV' - ['Potentiostat_U', 'Potentiostat_I_potstat']\n\t'Voltage_Loop_Down' for (start: max_V, stop: min_V, points: stepnumber):\n\t\tSet Channels 'Set_U_1' - {'Potentiostat_U': 'Voltage_Loop_Down_Value'}\n\t\tWait 'Wait_1' - waitingtime s\n\t\tRead Channels 'Read_CV_1' - ['Potentiostat_U', 'Potentiostat_I_potstat']\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/potentiostatprotocols/Protocol_CV_Stat.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(Protocol_CV_Stat_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(3011)
