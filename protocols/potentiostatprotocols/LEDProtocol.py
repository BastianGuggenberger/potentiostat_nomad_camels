...
live_windows = []
app = None
save_path = "C:/Users/GuggenbergerB/Documents/NOMAD_CAMELS_data/default_user/testsample/Testfile_CV_Test"
session_name = ""
export_to_csv = False
...
ledswitchnumber = 10
namespace["ledswitchnumber"] = ledswitchnumber
Voltage_Loop_Up_Count = 0
namespace["Voltage_Loop_Up_Count"] = Voltage_Loop_Up_Count
Voltage_Loop_Up_Value = 0
namespace["Voltage_Loop_Up_Value"] = Voltage_Loop_Up_Value

LEDProtocol_variable_signal = variable_reading.Variable_Signal(name="LEDProtocol_variable_signal", variables_dict=namespace)
aliases = {}
eva = Evaluator(namespace=namespace, aliases=aliases)



sys.path.append(r"C:\Users\GuggenbergerB\AppData\Local\Programs\NOMAD-CAMELS\camels_drivers\Potentiostat")
...
def LEDProtocol_plan_inner(devs, stream_name="primary", runEngine=None):
    global Voltage_Loop_Up_Count, Voltage_Loop_Up_Value, deltaV, max_V, min_V, waitingtime, ledswitchnumber
    ...
    for Voltage_Loop_Up_Count, Voltage_Loop_Up_Value in enumerate(helper_functions.get_range(eva, "min_V", "max_V", "100", "nan", "nan", "start - stop", "linear", "True", "deltaV", True)):
        ...
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        if eva.eval("Voltage_Loop_Up_Count%ledswitchnumber == 0"):

            """If"""
            helper_functions.update_protocol_counter(protocol_step_information)
            yield from bps.checkpoint()
            if eva.eval("(Voltage_Loop_Up_Count/ledswitchnumber)%2 == 0"):

                """ledon"""
                helper_functions.update_protocol_counter(protocol_step_information)
                yield from bps.checkpoint()
                yield from bps.abs_set(devs["xy"].led_intensity, eva.eval("255"), group="A")
                yield from bps.wait("A")
            else:
                ...
                helper_functions.update_protocol_counter(protocol_step_information)
                yield from bps.checkpoint()
                yield from bps.abs_set(devs["xy"].led_intensity, eva.eval("0"), group="A")
                yield from bps.wait("A")
        ...
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        channels_Read_CV = [devs["Potentiostat"].I, devs["Potentiostat"].U, LEDProtocol_variable_signal]
...
def LEDProtocol_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from LEDProtocol_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 1161
    ...
    md["protocol_overview"] = """'Voltage_Loop_Up' for (start: min_V, stop: max_V, points: 100):\n\tSet Channels 'Set_U' - {'Potentiostat_U': 'Voltage_Loop_Up_Value'}\n\tif Voltage_Loop_Up_Count%ledswitchnumber == 0:\n\t\tif (Voltage_Loop_Up_Count/ledswitchnumber)%2 == 0:\n\t\t\tSet Channels 'ledon' - {'xy_led_intensity': '255'}\n\t\telse:\n\t\t\tSet Channels 'ledoff' - {'xy_led_intensity': '0'}\n\tWait 'Wait' - waitingtime s\n\tRead Channels 'Read_CV' - ['Potentiostat_I', 'Potentiostat_U']\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/potentiostatprotocols/LEDProtocol.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(LEDProtocol_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(1161)
    ...
    try:
        ...
        """
        settings = {}
        additional_info = {'config_channel_metadata': {}, 'description': '', 'ELN-instrument-id': '1', 'ELN-service': '', 'ELN-metadata': {}, 'device_class_name': 'Potentiostat'}
        ...
        config = {'POTENTIOSTAT_COM_PORT': 'COM6'}
