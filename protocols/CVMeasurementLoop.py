...
cleaningiterations = 2
namespace["cleaningiterations"] = cleaningiterations
pointnumber = 0
namespace["pointnumber"] = pointnumber
samplepoints = "25.5,73.5;33.0,73.5"
namespace["samplepoints"] = samplepoints
For_Loop_Count = 0
namespace["For_Loop_Count"] = For_Loop_Count
For_Loop_Value = 0
namespace["For_Loop_Value"] = For_Loop_Value

CVMeasurementLoop_variable_signal = variable_reading.Variable_Signal(name="CVMeasurementLoop_variable_signal", variables_dict=namespace)
aliases = {}
...
spec = importlib.util.spec_from_file_location("CVMeasurement_noclean", "C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/CVMeasurement_noclean.py")
CVMeasurement_noclean_mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = CVMeasurement_noclean_mod
spec.loader.exec_module(CVMeasurement_noclean_mod)
...
def CVMeasurementLoop_plan_inner(devs, stream_name="primary", runEngine=None):
    global For_Loop_Count, For_Loop_Value, cleaningiterations, pointnumber, samplepoints

    """setpoints"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].samplepoints, eva.eval("samplepoints"), group="A")
    yield from bps.wait("A")

    """pointnumber"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    channels_pointnumber = [devs["xy"].samplepointnumber, CVMeasurementLoop_variable_signal]
    yield from helper_functions.trigger_and_read(channels_pointnumber, name=stream_name if stream_name == stream_name else f"{stream_name}||sub_stream||stream_name", skip_on_exception=[False, False])

    """pointnumber"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    pointnumber = eva.eval("xy_samplepointnumber")
    namespace["pointnumber"] = pointnumber

    """For_Loop"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    for For_Loop_Count, For_Loop_Value in enumerate(helper_functions.get_range(eva, "0", "pointnumber-1", "9", "nan", "nan", "start - stop", "linear", "True", "1", True)):
        namespace.update({"For_Loop_Count": For_Loop_Count, "For_Loop_Value": For_Loop_Value})

        """CV_Measurement"""
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        CVMeasurement_noclean_mod.protocol_step_information = protocol_step_information
        CVMeasurement_noclean_mod.index = eva.eval("For_Loop_Value")
        CVMeasurement_noclean_mod.namespace["index"] = eva.eval("For_Loop_Value")
        sub_eva_CVMeasurement_noclean = runEngine.subscribe(CVMeasurement_noclean_mod.eva)
        yield from CVMeasurement_noclean_mod.CVMeasurement_noclean_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_CV_Measurement", runEngine)
        runEngine.unsubscribe(sub_eva_CVMeasurement_noclean)
        yield from helper_functions.get_fit_results(CVMeasurement_noclean_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_CV_Measurement")

    """CleanHead"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    clean_head_mod.protocol_step_information = protocol_step_information
    clean_head_mod.iterations = eva.eval("cleaningiterations")
    clean_head_mod.namespace["iterations"] = eva.eval("cleaningiterations")
...
def CVMeasurementLoop_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from CVMeasurementLoop_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = CVMeasurement_noclean_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_CV_Measurement")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_CV_Measurement_steps"] = CVMeasurement_noclean_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_CV_Measurement")
    if "subs" not in returner:
        returner["subs"] = []
    if "plots" not in returner:
        returner["plots"] = []
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 2518
    ...
    md["protocol_overview"] = """Set Channels 'setpoints' - {'xy_samplepoints': 'samplepoints'}\nRead Channels 'pointnumber' - ['xy_samplepointnumber']\nSet Variables 'pointnumber' - {'Variable': ['pointnumber'], 'Value': ['xy_samplepointnumber']}\n'For_Loop' for (start: 0, stop: pointnumber-1, points: 9):\n\tRun Subprotocol 'CV_Measurement' - C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/CVMeasurement_noclean.cprot - \nRun Subprotocol 'CleanHead' - C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/clean_head.cprot - \n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/CVMeasurementLoop.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(CVMeasurementLoop_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(2518)
