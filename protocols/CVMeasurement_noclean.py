...
CVMeasurement_noclean_variable_signal = variable_reading.Variable_Signal(name="CVMeasurement_noclean_variable_signal", variables_dict=namespace)
aliases = {}
...
spec = importlib.util.spec_from_file_location("approach_sample_index_string", "C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/approach_sample_index_string.py")
approach_sample_index_string_mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = approach_sample_index_string_mod
spec.loader.exec_module(approach_sample_index_string_mod)
...
def CVMeasurement_noclean_plan_inner(devs, stream_name="primary", runEngine=None):
    global index
    ...
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    approach_sample_index_string_mod.protocol_step_information = protocol_step_information
    approach_sample_index_string_mod.positionindex = eva.eval("index")
    approach_sample_index_string_mod.namespace["positionindex"] = eva.eval("index")
    sub_eva_approach_sample_index_string = runEngine.subscribe(approach_sample_index_string_mod.eva)
    yield from approach_sample_index_string_mod.approach_sample_index_string_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_approachsample", runEngine)
    runEngine.unsubscribe(sub_eva_approach_sample_index_string)
    yield from helper_functions.get_fit_results(approach_sample_index_string_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_approachsample")
...
def CVMeasurement_noclean_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from CVMeasurement_noclean_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    if "plots_plotly" not in returner:
        returner["plots_plotly"] = []
    plots, subs, app, plots_plotly = approach_sample_index_string_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_approachsample")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_approachsample_steps"] = approach_sample_index_string_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_approachsample")
    if "subs" not in returner:
        returner["subs"] = []
    if "plots" not in returner:
        returner["plots"] = []
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    try:
        RE(CVMeasurement_noclean_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(1133)
