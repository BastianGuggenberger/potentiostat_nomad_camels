...
discard_solution_variable_signal = variable_reading.Variable_Signal(name="discard_solution_variable_signal", variables_dict=namespace)
aliases = {}
...
def discard_solution_plan_inner(devs, stream_name="primary", runEngine=None):
    ...
    yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_movewaste", runEngine)
    runEngine.unsubscribe(sub_eva_move)
    yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_movewaste")
    ...
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].purge_head_param, eva.eval("'PURGE_VOLUME'"), group="A")
    yield from bps.wait("A")
...
def discard_solution_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from discard_solution_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_movewaste")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_movewaste_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_movewaste")
    return returner
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    md["protocol_overview"] = """Set Channels 'change_volume' - {'xy_change_volume_param': "'NEGATIVE_VOLUME'"}\nRun Subprotocol 'movewaste' - C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/move.cprot - \nSet Channels 'heightwaste' - {'xy_height_param': "'WASTE_HEIGHT'"}\nSet Channels 'purge_head' - {'xy_purge_head_param': "'PURGE_VOLUME'"}\nSet Channels 'height_final' - {'xy_height_param': "'MOVE_HEIGHT'"}\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/discard_solution.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(discard_solution_plan(devs, md=md, runEngine=RE))
    finally:
