...
draw_solution_finalize_variable_signal = variable_reading.Variable_Signal(name="draw_solution_finalize_variable_signal", variables_dict=namespace)
aliases = {}
...
spec = importlib.util.spec_from_file_location("move", "C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/move.py")
move_mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = move_mod
...
def draw_solution_finalize_plan_inner(devs, stream_name="primary", runEngine=None):

    """movecleaningpos"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    move_mod.protocol_step_information = protocol_step_information
    move_mod.position = eva.eval("'CLEANING_POSITION'")
    move_mod.namespace["position"] = eva.eval("'CLEANING_POSITION'")
    sub_eva_move = runEngine.subscribe(move_mod.eva)
    yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_movecleaningpos", runEngine)
    runEngine.unsubscribe(sub_eva_move)
    yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_movecleaningpos")

    """heightcleaningheight"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].height_param, eva.eval("'CLEANING_HEIGHT'"), group="A")
    ...
    yield from bps.checkpoint()
    move_mod.protocol_step_information = protocol_step_information
    move_mod.position = eva.eval("'WASTE_POSITION'")
    move_mod.namespace["position"] = eva.eval("'WASTE_POSITION'")
    sub_eva_move = runEngine.subscribe(move_mod.eva)
    yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_movewastepos", runEngine)
    runEngine.unsubscribe(sub_eva_move)
    yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_movewastepos")

    """heightwaste"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].height_param, eva.eval("'WASTE_HEIGHT'"), group="A")
    yield from bps.wait("A")

    """fillhead"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].fill_head_param, eva.eval("'MEASUREMENT_VOLUME'"), group="A")
    yield from bps.wait("A")

    """changevolume"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].change_volume_param, eva.eval("'NEGATIVE_VOLUME'"), group="A")
    yield from bps.wait("A")

    """sheddrop"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    channels_sheddrop = [devs["xy"].shed_drop, draw_solution_finalize_variable_signal]
    yield from helper_functions.trigger_and_read(channels_sheddrop, name=stream_name if stream_name == stream_name else f"{stream_name}||sub_stream||stream_name", skip_on_exception=[False, False])

    """heightfinal"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].height_param, eva.eval("'MOVE_HEIGHT'"), group="A")
    yield from bps.wait("A")



def draw_solution_finalize_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from draw_solution_finalize_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_movecleaningpos")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_movecleaningpos_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_movecleaningpos")
    if "subs" not in returner:
        returner["subs"] = []
    if "plots" not in returner:
        returner["plots"] = []
    if "plots_plotly" not in returner:
        returner["plots_plotly"] = []
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_movewastepos")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_movewastepos_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_movewastepos")
    return returner
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 12
    ...
    md["protocol_overview"] = """Run Subprotocol 'movecleaningpos' - C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/move.cprot - \nSet Channels 'heightcleaningheight' - {'xy_height_param': "'CLEANING_HEIGHT'"}\nRun Subprotocol 'movewastepos' - C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/move.cprot - \nSet Channels 'heightwaste' - {'xy_height_param': "'WASTE_HEIGHT'"}\nSet Channels 'fillhead' - {'xy_fill_head_param': "'MEASUREMENT_VOLUME'"}\nSet Channels 'changevolume' - {'xy_change_volume_param': "'NEGATIVE_VOLUME'"}\nRead Channels 'sheddrop' - ['xy_shed_drop']\nSet Channels 'heightfinal' - {'xy_height_param': "'MOVE_HEIGHT'"}\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/draw_solution_finalize.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(draw_solution_finalize_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(12)
