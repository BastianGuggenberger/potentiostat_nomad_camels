...
approach_sample_index_variable_signal = variable_reading.Variable_Signal(name="approach_sample_index_variable_signal", variables_dict=namespace)
aliases = {}
...
def approach_sample_index_plan_inner(devs, stream_name="primary", runEngine=None):
    global pointnumber, positionindex

    """setpointnumber"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].samplepointnumber, eva.eval("pointnumber"), group="A")
...
def approach_sample_index_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from approach_sample_index_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    try:
        RE(approach_sample_index_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(9)
