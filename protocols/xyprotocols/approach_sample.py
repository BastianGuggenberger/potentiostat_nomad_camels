...
approach_sample_variable_signal = variable_reading.Variable_Signal(name="approach_sample_variable_signal", variables_dict=namespace)
aliases = {}
...
def approach_sample_plan_inner(devs, stream_name="primary", runEngine=None):
    ...
    move_mod.position = eva.eval("'ABSOLUTE_POSITION'")
    move_mod.namespace["position"] = eva.eval("'ABSOLUTE_POSITION'")
    sub_eva_move = runEngine.subscribe(move_mod.eva)
    yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_moveabsolutepos", runEngine)
    runEngine.unsubscribe(sub_eva_move)
    yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_moveabsolutepos")

    """approachheight"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].height_param, eva.eval("'APPROACH_SAMPLE_HEIGHT'"), group="A")
    yield from bps.wait("A")

    """approach"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].approach_param, eva.eval("'TARGET_GRAM_FORCE'"), group="A")
    yield from bps.wait("A")



def approach_sample_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from approach_sample_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_moveabsolutepos")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_moveabsolutepos_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_moveabsolutepos")
    return returner
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 5
    md["user"] = {'name': 'default_user'}
    md["sample"] = {'name': 'default_sample'}
    md["session_name"] = session_name
    md["protocol_overview"] = """Run Subprotocol 'moveabsolutepos' - C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/move.cprot - \nSet Channels 'approachheight' - {'xy_height_param': "'APPROACH_SAMPLE_HEIGHT'"}\nSet Channels 'approach' - {'xy_approach_param': "'TARGET_GRAM_FORCE'"}\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/approach_sample.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(approach_sample_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(5)
