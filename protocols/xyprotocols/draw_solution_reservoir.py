...
draw_solution_reservoir_variable_signal = variable_reading.Variable_Signal(name="draw_solution_reservoir_variable_signal", variables_dict=namespace)
aliases = {}
...
spec = importlib.util.spec_from_file_location("draw_solution_finalize", "C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/draw_solution_finalize.py")
draw_solution_finalize_mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = draw_solution_finalize_mod
spec.loader.exec_module(draw_solution_finalize_mod)



def draw_solution_reservoir_plan_inner(devs, stream_name="primary", runEngine=None):
    ...
    move_mod.position = eva.eval("'RESERVOIR_POSITION'")
    move_mod.namespace["position"] = eva.eval("'RESERVOIR_POSITION'")
    sub_eva_move = runEngine.subscribe(move_mod.eva)
    yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_movereservoir", runEngine)
    runEngine.unsubscribe(sub_eva_move)
    yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_movereservoir")

    """heightreservoir"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].height_param, eva.eval("'RESERVOIR_HEIGHT'"), group="A")
    yield from bps.wait("A")

    """fillhead"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].fill_head_param, eva.eval("'TOTAL_VOLUME'"), group="A")
    yield from bps.wait("A")

    """draw_solution_finalize"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    draw_solution_finalize_mod.protocol_step_information = protocol_step_information
    sub_eva_draw_solution_finalize = runEngine.subscribe(draw_solution_finalize_mod.eva)
    yield from draw_solution_finalize_mod.draw_solution_finalize_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_draw_solution_finalize", runEngine)
    runEngine.unsubscribe(sub_eva_draw_solution_finalize)
    yield from helper_functions.get_fit_results(draw_solution_finalize_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_draw_solution_finalize")



def draw_solution_reservoir_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from draw_solution_reservoir_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_movereservoir")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_movereservoir_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_movereservoir")
    if "subs" not in returner:
        returner["subs"] = []
    if "plots" not in returner:
        returner["plots"] = []
    if "plots_plotly" not in returner:
        returner["plots_plotly"] = []
    plots, subs, app, plots_plotly = draw_solution_finalize_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_draw_solution_finalize")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_draw_solution_finalize_steps"] = draw_solution_finalize_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_draw_solution_finalize")
    return returner
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 17
    md["user"] = {'name': 'default_user'}
    md["sample"] = {'name': 'default_sample'}
    md["session_name"] = session_name
    md["protocol_overview"] = """Run Subprotocol 'movereservoir' - C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/move.cprot - \nSet Channels 'heightreservoir' - {'xy_height_param': "'RESERVOIR_HEIGHT'"}\nSet Channels 'fillhead' - {'xy_fill_head_param': "'TOTAL_VOLUME'"}\nRun Subprotocol 'draw_solution_finalize' - C:/Users/GuggenbergerB/AppData/Local/nomad_camels/python_files/draw_solution_finalize.cprot - \n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/draw_solution_reservoir.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(draw_solution_reservoir_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(17)
