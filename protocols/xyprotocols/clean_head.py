...
clean_head_variable_signal = variable_reading.Variable_Signal(name="clean_head_variable_signal", variables_dict=namespace)
aliases = {}
...
spec = importlib.util.spec_from_file_location("move", "C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/move.py")
...
def clean_head_plan_inner(devs, stream_name="primary", runEngine=None):
    global iterations

    """For_Loop"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    for For_Loop_Count, For_Loop_Value in enumerate(helper_functions.get_range(eva, "1", "iterations", "0", "nan", "nan", "start - stop", "linear", "True", "1", True)):
        ...
        yield from move_mod.move_plan_inner(devs, f"{stream_name}||subprotocol_stream||Subprotocol_move", runEngine)
        runEngine.unsubscribe(sub_eva_move)
        yield from helper_functions.get_fit_results(move_mod.all_fits, namespace, True, f"{stream_name}||subprotocol_stream||Subprotocol_move")
        ...
        helper_functions.update_protocol_counter(protocol_step_information)
        yield from bps.checkpoint()
        yield from bps.abs_set(devs["xy"].fill_head_param, eva.eval("'CLEANING_VOLUME'"), group="A")
        yield from bps.wait("A")
...
def clean_head_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from clean_head_plan_inner(devs, stream_name, runEngine)
...
def steps_add_main(RE, devs, stream="primary"):
    ...
    plots, subs, app, plots_plotly = move_mod.create_plots(RE, f"{stream}||subprotocol_stream||Subprotocol_move")
    returner["subs"] += subs
    returner["plots"] += plots
    if plots_plotly:
        returner["plots_plotly"] += plots_plotly
    returner["Subprotocol_move_steps"] = move_mod.steps_add_main(RE, devs, f"{stream}||subprotocol_stream||Subprotocol_move")
    if "subs" not in returner:
        returner["subs"] = []
    if "plots" not in returner:
        returner["plots"] = []
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 27
    md["user"] = {'name': 'default_user'}
    md["sample"] = {'name': 'default_sample'}
    md["session_name"] = session_name
    md["protocol_overview"] = """'For_Loop' for (start: 1, stop: iterations, points: 0):\n\tRun Subprotocol 'move' - C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/move.cprot - \n\tSet Channels 'heightclean' - {'xy_height_param': "'CLEANING_HEIGHT'"}\n\tSet Channels 'fillhead' - {'xy_fill_head_param': "'CLEANING_VOLUME'"}\n\tRun Subprotocol 'discardsolution' - C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/discard_solution.cprot - \n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/clean_head.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(clean_head_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(27)
