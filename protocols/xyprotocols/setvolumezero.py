...
setvolumezero_variable_signal = variable_reading.Variable_Signal(name="setvolumezero_variable_signal", variables_dict=namespace)
aliases = {}
...
def setvolumezero_plan_inner(devs, stream_name="primary", runEngine=None):
    ...
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].position_param, eva.eval("'WASTE_POSITION'"), group="A")
    yield from bps.wait("A")

    """Read_Channels"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    channels_Read_Channels = [devs["xy"].volume_zero, setvolumezero_variable_signal]
    yield from helper_functions.trigger_and_read(channels_Read_Channels, name=stream_name if stream_name == stream_name else f"{stream_name}||sub_stream||stream_name", skip_on_exception=[False, False])



def setvolumezero_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from setvolumezero_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    md["protocol_overview"] = """Set Channels 'height' - {'xy_height_param': "'MOVE_HEIGHT'"}\nSet Channels 'position' - {'xy_position_param': "'WASTE_POSITION'"}\nRead Channels 'Read_Channels' - ['xy_volume_zero']\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/setvolumezero.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(setvolumezero_plan(devs, md=md, runEngine=RE))
    finally:
