...
positionindex = 4
namespace["positionindex"] = positionindex

approach_sample_index_string_variable_signal = variable_reading.Variable_Signal(name="approach_sample_index_string_variable_signal", variables_dict=namespace)
aliases = {}
...
def approach_sample_index_string_plan_inner(devs, stream_name="primary", runEngine=None):
    global pointnumber, positionindex
    ...
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].position_drop, eva.eval("positionindex"), group="A")
    yield from bps.wait("A")
    ...
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    yield from bps.abs_set(devs["xy"].position_sample, eva.eval("positionindex"), group="A")
    yield from bps.wait("A")
...
def approach_sample_index_string_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from approach_sample_index_string_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    protocol_step_information["total_protocol_steps"] = 8
    md["user"] = {'name': 'default_user'}
    md["sample"] = {'name': 'default_sample'}
    md["session_name"] = session_name
    md["protocol_overview"] = """Set Channels 'height' - {'xy_height_param': "'MOVE_HEIGHT'"}\nSet Channels 'droppos' - {'xy_position_drop': 'positionindex'}\nSet Channels 'apheight' - {'xy_height_param': "'APPROACH_SAMPLE_HEIGHT'"}\nSet Channels 'apr' - {'xy_approach_param': "'TARGET_GRAM_FORCE'"}\nSet Channels 'mvheight' - {'xy_height_param': "'MOVE_HEIGHT'"}\nSet Channels 'samplepos' - {'xy_position_sample': 'positionindex'}\nSet Channels 'approachheight' - {'xy_height_param': "'APPROACH_SAMPLE_HEIGHT'"}\nSet Channels 'approach' - {'xy_approach_param': "'TARGET_GRAM_FORCE'"}\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/approach_sample_index_string.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(approach_sample_index_string_plan(devs, md=md, runEngine=RE))
    finally:
...
def main(dispatcher=None, publisher=None):
    ...
    tqdm_bar = tqdm_progress_bar.ProgressBar(8)
