...
live_windows = []
app = None
save_path = "C:/Users/GuggenbergerB/Documents/NOMAD_CAMELS_data/default_user/Testsample/home_data"
session_name = ""
export_to_csv = False
...
home_variable_signal = variable_reading.Variable_Signal(name="home_variable_signal", variables_dict=namespace)
aliases = {}
...
def home_plan_inner(devs, stream_name="primary", runEngine=None):

    """home"""
    helper_functions.update_protocol_counter(protocol_step_information)
    yield from bps.checkpoint()
    channels_home = [devs["xy"].home, home_variable_signal]
    yield from helper_functions.trigger_and_read(channels_home, name=stream_name if stream_name == stream_name else f"{stream_name}||sub_stream||stream_name", skip_on_exception=[False, False])



def home_plan(devs, md=None, runEngine=None, stream_name="primary"):
    sub_eva = runEngine.subscribe(eva)
    yield from bps.open_run(md=md)

    if web_ports:
        yield from wait_for_dash_ready_plan(web_ports)
    yield from home_plan_inner(devs, stream_name, runEngine)
...
def run_protocol_main(RE, dark=False, used_theme="default", catalog=None, devices=None, md=None, dispatcher=None, publisher=None, additionals=None):
    ...
    md["protocol_overview"] = """Read Channels 'home' - ['xy_home']\n"""
    md["description"] = ''
    md["measurement_tags"] = []
    md["measurement_description"] = ''
    try:
        with open("C:/Users/GuggenbergerB/AppData/Local/Programs/NOMAD-CAMELS/protocols/xyprotocols/home.cprot", "r", encoding="utf-8") as f:
    ...
    try:
        RE(home_plan(devs, md=md, runEngine=RE))
    finally:
